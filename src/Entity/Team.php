...
class Team
{
     ...
     * @return Collection<int, Game>
     */
    public function getGamesB(): Collection
    {
        return $this->games_b;
    }

    public function addGamesB(Game $gamesB): self
    {
        if (!$this->games_b->contains($gamesB)) {
            $this->games_b->add($gamesB);
            $gamesB->setTeamB($this);
        }

        return $this;
    }

    public function removeGamesB(Game $gamesB): self
    {
        if ($this->games_b->removeElement($gamesB)) {
            // set the owning side to null (unless already changed)
            if ($gamesB->getTeamB() === $this) {
